...
def get_matatag_lab_config(node_id: str):
    ...
    for axis in axes:
        ...
        if dim_type == "continuous":
            ...
            # Filter regrouping options by max operand value from competency
            ...
            # Filter number_range options by max_minuend value from competency
            elif axis_name == "number_range" and primary_concept == "subtraction":
                max_minuend = None
                if "max_minuend" in competency_bounds:
                    bounds_tuple = competency_bounds["max_minuend"]
                    if isinstance(bounds_tuple, tuple) and len(bounds_tuple) == 2:
                        max_minuend = bounds_tuple[1]

                # Filter to only show options up to (and slightly beyond) the specified bound
                if max_minuend:
                    filtered_levels = [
                        lv for lv in levels
                        if lv.get("value", 10000) >= max_minuend
                    ]
                    if filtered_levels:
                        levels = filtered_levels
