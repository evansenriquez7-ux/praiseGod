...
4. **Authority, not permission — build what the curriculum requires.** MATATAG is the only source of what content *becomes*. Change content freely **toward** the written curriculum; never **past** it. If a competency names a verb, sub-case, or range the pipeline cannot produce, then adding the formatter, variant, dd, or DNA that produces it **is the fix** — not scope creep, and never a reason to defer the node. If a competency does *not* name it, building it is invention and is forbidden however good the idea. Cite the competency clause each such change conforms to, in the commit and the evidence log. How: `docs/extending_pgen.md`. (This supersedes `pgen_hardening.md`'s "Explicit non-goals" — they scoped that finished plan and now contradict the judgment gate it built. A previous agent correctly declined real work by citing them.)
