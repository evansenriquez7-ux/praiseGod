  ...
  return (
    <div className="app-container">
      ...
      <main style={{ flex: 1, padding: '40px', maxWidth: '1400px', width: '100%', margin: '0 auto' }}>
        ...
        {currentView === 'login' && (
          <>
            {/* Backend Connection Status Manager */}
            <div className="glass-card" style={{ 
              marginBottom: '30px', 
              borderLeft: connectionStatus === 'connected' ? '4px solid #10b981' : (connectionStatus === 'checking' ? '4px solid #3b82f6' : '4px solid #ef4444'), 
              padding: '20px 25px', 
              display: 'flex', 
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '15px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  {connectionStatus === 'checking' && (
                    <span className="badge-status active" style={{ background: 'rgba(59, 130, 246, 0.15)', color: '#60a5fa', border: '1px solid rgba(59, 130, 246, 0.25)' }}>⚡ Checking Connection...</span>
                  )}
                  {connectionStatus === 'connected' && (
                    <span className="badge-status mastered" style={{ background: 'rgba(16, 185, 129, 0.15)', color: '#34d399', border: '1px solid rgba(16, 185, 129, 0.25)' }}>🟢 Online</span>
                  )}
                  {connectionStatus === 'error' && (
                    <span className="badge-status locked" style={{ background: 'rgba(239, 68, 68, 0.15)', color: '#fca5a5', border: '1px solid rgba(239, 68, 68, 0.25)' }}>🔴 Offline / Disconnected</span>
                  )}
                  
                  <button 
                    className="btn-secondary" 
                    onClick={() => setShowApiEditor(prev => !prev)}
                    style={{ padding: '8px 14px', fontSize: '13px', borderRadius: '10px', height: '36px', display: 'flex', alignItems: 'center', gap: '4px' }}
                  >
                    <span>⚙️ Configure Server</span>
                  </button>
                </div>
              </div>

              {connectionStatus === 'error' && !showApiEditor && (
                <div style={{ 
                  ...
                  ⚠️ <b>Connection Timeout:</b> The frontend client is currently unable to communicate with the academic mastery backend database at <code>{API_BASE}</code>.<br/>
                  {window.location.hostname.endsWith('here.now') ? (
                    <span style={{ display: 'inline-block', marginTop: '6px' }}>Since you are accessing CCMed from an external network via <b>here.now</b>, you must ensure your <b>Tailscale Funnel</b> is active. If you recently changed the backend, click <b>Configure Server</b> and <b>Reset Default</b> to clear old tunnel URLs.</span>
                  ) : (
                    <span style={{ display: 'inline-block', marginTop: '6px' }}>Please verify that your FastAPI backend is running locally on port 8000. Run <code>./manage.sh start</code> on your host machine to activate it.</span>
                  )}
                </div>
              )}
              ...
              {showApiEditor && (
                <div style={{ 
                  ...
                  <label style={{ fontSize: '13.5px', fontWeight: 600, color: '#f1f5f9' }}>Set Pinned Backend API Base URL:</label>
                  <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                    <input 
                      type="text" 
                      className="premium-input" 
                      placeholder="e.g. https://ccmed-backend-persistent.loca.lt" 
                      value={customApiUrl}
                      onChange={(e) => setCustomApiUrl(e.target.value)}
                      style={{ flex: 1, minWidth: '280px', height: '42px', fontSize: '14px', padding: '0 15px' }}
                    />
                    <button 
                      className="btn-primary" 
                      onClick={() => {
                        let formatted = customApiUrl.trim();
                        ...
                        localStorage.setItem('ccmed_api_base', formatted);
                        window.location.reload();
                      }}
                      style={{ padding: '0 22px', height: '42px', fontSize: '14px', fontWeight: 600 }}
                    >
                      💾 Save & Connect
                    </button>
                    <button 
                      className="btn-secondary" 
                      onClick={() => {
                        localStorage.removeItem('ccmed_api_base');
                        window.location.reload();
                      }}
                      style={{ padding: '0 18px', height: '42px', fontSize: '13.5px', borderColor: 'rgba(239, 68, 68, 0.3)', color: '#f87171', background: 'rgba(239, 68, 68, 0.05)' }}
                    >
                      🔄 Reset Default
                    </button>
                  </div>
