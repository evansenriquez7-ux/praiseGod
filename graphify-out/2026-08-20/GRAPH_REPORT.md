...
## Corpus Check
- 453 files · ~728,291 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3416 nodes · 5430 edges · 262 communities (251 shown, 11 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 89 edges (avg confidence: 0.57)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `e6929ed2`
...
## Communities (262 total, 11 thin omitted)
...
Cohesion: 0.06
Nodes (64): get_db(), FastAPI dependency that yields a database session.     Guarantees session closur, SessionLocal(), _combined_interests(), health(), Merge parent-set interest_tags and student-set student_interest_tags into a, Background task to pre-generate questions into the cache.     Uses parallel exec, Liveness probe. Reaching this proves the whole module graph imported and     uvi (+56 more)
...
Cohesion: 0.07
Nodes (36): _answer_leaks_into_stem(), _normalize_stem(), _numeric_payload_values(), _option_in_stem(), _profile_echo_keys(), Every `given_values` key that is an echo of the request rather than generated, Every plain number a generated problem exposes to the student, labelled for, Lowercase the stem and render LaTeX fractions as 'a/b' so an option like     '1/ (+28 more)
...
Cohesion: 0.09
Nodes (35): find_node_id(), get_all_node_ids(), get_node_dnas(), get_node_formatters(), Practice Generation — Node Registry ======================================  Maps, # NOTE: `max_difference` was removed from this list on 2026-07-01., Return the list of DNA concept names for a node.      Args:         node_id: MAT, Return the union of compatible formatters from all DNAs for a node.      Looks u (+27 more)
...
Cohesion: 0.24
Nodes (9): generate_hints(), generate_params(), _inverse_op(), Any, DNA: Missing Number (Number & Algebra)  Covers MATATAG grades 1–3 missing-number, Rejection-sample a missing-number equation matching difficulty_profile.      Ret, RuntimeError, AuditHarnessError (+1 more)

### Community 103 - "Community 103"
Cohesion: 0.10
Nodes (21): 1. Fractions answer-key false positive — `mat_g2_na_q4_0`/`_1`/`_2`, 2026-08-13 — length_measurement: the largest FAIL cluster in the tree, 2026-08-13 — The G1 measure items: a visual for the task, and a size model for the units, 2026-08-14 — "2-digit by 1-digit" bounds two operands, and only one had a key, 2026-08-19 — Unit 3 completion + full `run_all` verification, 2. `§1A-reach` false failure — `mat_g3_na_q2_0` (money_peso), 3. KG monotonicity — `order_of_operations` introduced but not propagated, A regression I introduced and caught by measuring (+13 more)
...
Cohesion: 0.17
Nodes (21): apply_experience(), apply_formatter(), generate_batch(), generate_problem(), _get_dna_instance(), Any, Random, Practice Generation — API Adapter ===================================  Main entr (+13 more)
...
Cohesion: 0.11
Nodes (16): Translate a FormattedProblem to the legacy skeleton dict format.      This shim, to_legacy_dict(), PracticeOrchestrator, Any, Bridge Layer separating the generation logic from the FastAPI routing/server lay, is_prompt_target_formatter(), The orchestrator must set problem.dna_name to the actually-chosen DNA., Cross-DNA formatter filter. RESTORED 2026-08-19 (hardening Unit 1): cc5977f5 (+8 more)
...
Cohesion: 0.08
Nodes (37): get_variants_for_dna(), is_variant_available_at(), Return all contextual variants defined for a DNA concept.      Args:         dna, Check if a variant is available at a specific grade/quarter per curriculum., _build_symbolic_question(), _detect_axes_served(), _eval_error_formula(), generate_context() (+29 more)
...
Cohesion: 0.10
Nodes (23): get_pipeline_status(), Any, Return a health-check dict for the pipeline.      Checks:       - Whether each o, Generate a single practice problem and return it as a dict.      This is the sin, run(), Exception, build(), main() (+15 more)
...
Cohesion: 0.07
Nodes (48): get_node_competency_bounds(), _parse_competency_bounds(), Parse competency text to extract bounds for difficulty dimensions.      Returns, Get competency-specific bounds for a node's difficulty dimensions.          Retu, Any, Registry to map visual types to their Pydantic schemas., VisualSchemaRegistry, _parse_contract_section_refs() (+40 more)
...
- **Why does `FormattedProblem` connect `2026-08-13 — perimeter: an impossible triangle, and two shapes that never appeared` to `generate_params`, `Community 131`, `Community 8`, `Community 13`, `Community 18`, `Community 153`, `Community 50`, `Community 51`, `Spine`, `Community 59`, `Community 60`, `validate_capability.py`, `get_gemini_models`, `Community 70`, `Community 77`, `Community 79`, `_get_available_formats`, `test_semantic_leak_guards.py`, `run_audit`, `Community 92`, `Community 93`, `Community 94`, `Community 95`, `fmt_numeric_input.py`, `Community 99`, `Community 100`, `CompetencyConfiguration`, `generate_params`?**
  _High betweenness centrality (0.031) - this node is a cross-community bridge._
- **Why does `DNA` connect `generate_params` to `Community 1`, `VisualSchemaRegistry`, `generate_params`, `generate_params`, `Community 138`, `Community 11`, `Community 14`, `Community 41`, `validate_interest.py`, `mastery_drill.py`, `Native SQL Examples`, `get_node_competency_bounds`, `fractions.py`, `2026-08-13 — perimeter: an impossible triangle, and two shapes that never appeared`, `Community 87`, `mass_capacity.py`, `get_all_node_ids`, `socratic_chat_exchange`, `Community 101`, `Community 102`, `TestLogScaleDeclarations`, `Community 104`, `Community 118`, `Community 123`?**
