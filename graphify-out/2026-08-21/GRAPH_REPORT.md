...
## Corpus Check
- 470 files · ~768,307 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3532 nodes · 5574 edges · 296 communities (270 shown, 26 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 90 edges (avg confidence: 0.57)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `7b2d8deb`
...
3. `DNA` - 57 edges
4. `Phase F — full `run_all` verification and three more regressions it caught — 2026-08-02` - 55 edges
...
Cohesion: 0.15
Nodes (20): _audit_node(), build_test_profiles(), _check_cap_vs_formatter_code(), _check_variant_sensitivity(), _check_visual_capacity(), extract_numeric_state(), get_lab_config(), _import_harness_dependencies() (+12 more)
...
Cohesion: 0.08
Nodes (43): _parse_contract_section_refs(), Practice Generation — Validation Harness Runner  Runs all validators in the prac, Extract every '§1A'-style reference from docs/pgen_contract.md's rule     table, Run the fast unit suite and report it as a harness stage (§0).      Subprocess r, run_all(), _run_unit_tests(), _are_values_equal(), _eval_formula() (+35 more)
...
Cohesion: 0.10
Nodes (17): Mutation tests for the capability contract (§6A/§6B/§6C).  These exist because g, Guards against the decoy that cost an earlier audit a day.      483 of 485 provi, The regression §6F exists to stop: someone re-registers what a blind Attester, A record that cannot be re-rendered is not evidence — and is never silently skip, A `bounds` list carried verbatim by most of the table claims nothing.      Measu, The loophole this whole design would otherwise have: drop the requirement you, Ignoring a competency word is possible but visible -- never a silent default., The acceptance test for the entire capability contract: mat_g3_mg_q1_5's     com (+9 more)
...
Cohesion: 0.16
Nodes (23): get_axes_for_concept(), Return the UI-ready axis list for a concept, or [] if not found., _bound_allows(), build_group(), build_packet(), _main(), _max_difficulty_profile(), Any (+15 more)
...
Cohesion: 0.13
Nodes (16): generate_hints(), generate_params(), Any, DNA: Addition (Number & Algebra)  Refactored from:   - matatag_skeletons.py  (ar, Generate (a, b) that satisfy the difficulty_profile constraints.      Uses smart, # NOTE: "structure" is a contextual variant, not a difficulty dimension., Return 2–4 step-by-step hint strings for the given addition problem., generate_hints() (+8 more)

### Community 103 - "Community 103"
Cohesion: 0.08
Nodes (26): 1. Fractions answer-key false positive — `mat_g2_na_q4_0`/`_1`/`_2`, 2026-08-13 — "Estimate" was only a word (mat_g3_mg_q1_0), 2026-08-13 — length_measurement: the largest FAIL cluster in the tree, 2026-08-14 — "2-digit by 1-digit" bounds two operands, and only one had a key, 2026-08-19 — Unit 3 completion + full `run_all` verification, 2026-08-20 (tick 8) — A dispatch-only tick: coverage 52 → 127, 2. `§1A-reach` false failure — `mat_g3_na_q2_0` (money_peso), 3. KG monotonicity — `order_of_operations` introduced but not propagated (+18 more)

### Community 104 - "Community 104"
Cohesion: 0.08
Nodes (39): get_node_info(), Return the full knowledge-graph node dict for a node.      Args:         node_id, _attestation_records(), _attestation_staleness(), _bound_restricts_to(), _content_words(), _load_attestations(), _nondiscriminating_bounds() (+31 more)
...
Cohesion: 0.10
Nodes (26): get_node(), Return the raw knowledge-graph node dict for node_id, or None.      Args:, load_dna(), Practice Generation — Validation Manifest  Central source of truth for the valid, Import the DNA module and return its DNA instance.     Raises ImportError if loa, Verify that every difficulty profile for a DNA meets MIN_ACCEPTANCE_RATE.      F, Run validate_difficulty_feasibility for every DNA x every grade present     in t, run_all_feasibility_checks() (+18 more)
...
Cohesion: 0.11
Nodes (19): ErrorPattern, A pedagogically meaningful wrong answer.      formula:          SymPy expression, generate_hints(), generate_params(), Any, DNA: Probability Experiment (Data & Probability)  Covers MATATAG grade 3 compete, Generate parameters for collecting and recording data from simple experiments., generate_hints() (+11 more)
...
Cohesion: 0.25
Nodes (12): _answer_leaks_into_stem(), _option_in_stem(), Whether an option value appears in the stem as a *whole* token.      Substring m, Return a description of the leak, or None.      A stem that contains its own ans, _problem(), Unit tests for the two contract checks added on 2026-07-27:   §1F  — a question, Ordering restates every value it asks the student to sort., test_detects_answer_leak() (+4 more)
...
Cohesion: 0.15
Nodes (17): A mathematical term fragment gated behind vocab knowledge.      preferred:    Te, VocabGated, generate_hints(), generate_params(), Any, DNA: Geometric Lines (Measurement & Geometry)  Covers MATATAG grade 2-3 lines/su, Sample one item from the static pool filtered by grade and difficulty profile., _build_traps() (+9 more)

### Community 268 - "generate_params"
Cohesion: 0.20
Nodes (7): PracticeOrchestrator, Bridge Layer separating the generation logic from the FastAPI routing/server lay, test_formatter_supports_profile.py =================================== Regressio, The orchestrator must set problem.dna_name to the actually-chosen DNA., The complementary case: when the formatter IS offered by the node's own DNA,, TestOrchestratorAnnotatesDnaName, TestDnasGenerateFullSpace
...
### Community 271 - "2026-08-13 — Content no competency asks for, and a metre wearing centimetres"
Cohesion: 0.40
...
### Community 274 - "_combined_interests"
Cohesion: 0.18
...
### Community 279 - "2026-08-20 (tick 4) — An exploitable answer-key pattern across the whole tree, and why it cannot be fixed yet"
Cohesion: 0.40
...
Cohesion: 0.33
Nodes (6): max_regrouping_places(), Largest carry count physically achievable for any (a, b) with a + b <= max_resul, get_matatag_lab_config(), _get_max_regrouping_places(), Max regrouping/borrowing places feasible for a given number range.      Thin wra, Return full lab configuration for a MATATAG node.      Includes:     - difficult
...
### Community 286 - "2026-08-20 — §5 worker death: a silent unbounded wait in the last gate"
Cohesion: 0.50
...
## Knowledge Gaps
- **1128 isolated node(s):** `graphify`, `PackageDescription`, `Foundation`, `PathKit`, `graphify-mcp` (+1123 more)
...
- **Why does `FormattedProblem` connect `models.py` to `generate_params`, `_get_available_formats`, `.axis_level_index`, `generate_params`, `Community 8`, `fmt_array_grid.py`, `generate_params`, `Community 13`, `Community 18`, `Community 153`, `Community 25`, `Community 41`, `Community 50`, `Community 51`, `Spine`, `get_node_competency_bounds`, `Community 59`, `Community 60`, `get_gemini_models`, `Community 70`, `Community 77`, `Community 79`, `Community 92`, `Community 93`, `Community 95`, `socratic_chat_exchange`, `CompetencyConfiguration`, `2026-08-14 — The blank landed on a given, not on the unknown`?**
  _High betweenness centrality (0.034) - this node is a cross-community bridge._
- **Why does `QuestionContext` connect `Community 50` to `generate_params`, `_get_available_formats`, `.axis_level_index`, `generate_params`, `generate_params`, `Community 13`, `Community 18`, `Community 153`, `Community 41`, `Community 51`, `Spine`, `get_node_competency_bounds`, `Community 59`, `Community 60`, `get_gemini_models`, `Community 70`, `Community 77`, `Community 79`, `Community 92`, `Community 93`, `models.py`, `Community 95`, `socratic_chat_exchange`, `CompetencyConfiguration`, `Community 117`?**
  _High betweenness centrality (0.030) - this node is a cross-community bridge._
- **Why does `DNA` connect `Native SQL Examples` to `Community 1`, `generate_params`, `2026-08-13 — length_measurement: the largest FAIL cluster in the tree`, `generate_params`, `Community 11`, `Community 14`, `2026-08-20 (tick 4) — An exploitable answer-key pattern across the whole tree, and why it cannot be fixed yet`, `Community 24`, `Community 36`, `Community 41`, `validate_interest.py`, `mastery_drill.py`, `2026-08-13 — perimeter: an impossible triangle, and two shapes that never appeared`, `_get_available_formats`, `Community 87`, `mass_capacity.py`, `models.py`, `Community 101`, `Community 102`, `RedisDict`, `Community 117`, `generate_params`, `Community 118`, `Part 3 — Concrete restructuring of the current docs/`, `Community 123`?**
