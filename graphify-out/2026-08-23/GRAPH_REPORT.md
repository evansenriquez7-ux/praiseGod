...
## Corpus Check
- 480 files · ~797,878 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3689 nodes · 5792 edges · 331 communities (302 shown, 29 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 91 edges (avg confidence: 0.57)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `d10bfb22`
...
- [[_COMMUNITY_frontend_contract_auditor.py|frontend_contract_auditor.py]]
- [[_COMMUNITY_fmt_error_detect.py|fmt_error_detect.py]]
- [[_COMMUNITY__combined_interests|_combined_interests]]
- [[_COMMUNITY_get_node_competency_bounds|get_node_competency_bounds]]
...
- [[_COMMUNITY__combined_interests|_combined_interests]]
- [[_COMMUNITY__numeric_payload_values|_numeric_payload_values]]
- [[_COMMUNITY_probability_language.py|probability_language.py]]
- [[_COMMUNITY_mastery_drill.py|mastery_drill.py]]
...
3. `Phase F — full `run_all` verification and three more regressions it caught — 2026-08-02` - 71 edges
4. `DNA` - 57 edges
5. `augment_distractors()` - 50 edges
6. `VocabGated` - 37 edges
7. `ErrorPattern` - 32 edges
8. `get_cache()` - 32 edges
9. `generate_pair_by_window()` - 30 edges
10. `get_node_dnas()` - 30 edges
...
## Communities (331 total, 29 thin omitted)
...
Cohesion: 0.07
Nodes (42): Translate a FormattedProblem to the legacy skeleton dict format.      This shim, to_legacy_dict(), get_supported_variants(), Return variants supported by a specific DNA + formatter combination.      Applie, get_matatag_lab_config(), Return full lab configuration for a MATATAG node.      Includes:     - difficult, _audit_node(), build_test_profiles() (+34 more)
...
Cohesion: 0.10
Nodes (13): RedisDict, RedisDict, get_matatag_difficulty_axes(), get_matatag_lab_interests(), matatag_lab_submit(), Return the difficulty axes for a specific MATATAG node.      Axes are specific t, Return ALL available interest themes for word problem personalization., Grade a MATATAG Lab answer.  Handles both visual and MCQ skeleton types.      Re (+5 more)
...
Cohesion: 0.10
Nodes (17): Mutation tests for the capability contract (§6A/§6B/§6C).  These exist because g, The discrimination must key on *what survives removing the family*, never on how, §6F must clear once a verdict is on file, or it is a counter rather than a check, There is no 'partly provided'. A malformed record is loud, never skipped., An attestation is evidence about specific rendered content and stops being evide, A record that cannot be re-rendered is not evidence — and is never silently skip, The loophole this whole design would otherwise have: drop the requirement you, The gate must not retroactively fail honest work. Measured 2026-08-23: 143 live (+9 more)
...
Cohesion: 0.15
Nodes (17): A mathematical term fragment gated behind vocab knowledge.      preferred:    Te, VocabGated, generate_hints(), generate_params(), Any, DNA: Probability Language (Data & Probability)  Covers MATATAG grade 3 probabili, Sample one scenario from the static pool filtered by difficulty profile., _build_traps() (+9 more)
...
Cohesion: 0.17
Nodes (18): _node_file(), _normalize(), _option_values(), _provenance_corpus(), Any, Path, Practice Generation — Judgment Review Validator (hard gate)  The judgment items, Return a list of schema/quality errors for one node's review file. (+10 more)
...
Cohesion: 0.08
Nodes (31): interpolate(), linear_interpolate(), log_interpolate(), Compute the dimension value at difficulty scalar t.          If override_min/ove, Logarithmic interpolation between min_val and max_val at position t.      Produc, Linear interpolation between min_val and max_val at position t., Interpolate between min_val and max_val using the given scale type.      scale_t, generate_hints() (+23 more)
...
Cohesion: 0.13
Nodes (22): Every span a rationale puts in quotes must exist in the review's own packet., One reviewer identity may not stamp the whole tree.      Blind review is dispatc, Fail any normalized rationale skeleton shared by more than _MAX_SKELETON_CLUSTER, _validate_quote_provenance(), _validate_reviewer_plurality(), _validate_skeleton_clusters(), test_judgment_antitemplate.py ============================= The judgment gate's, A CONCERN or FAIL verdict must not exempt a review from the content checks. (+14 more)

### Community 103 - "Community 103"
Cohesion: 0.08
Nodes (26): 1. Fractions answer-key false positive — `mat_g2_na_q4_0`/`_1`/`_2`, 2026-08-13 — length_measurement: the largest FAIL cluster in the tree, 2026-08-14 — "2-digit by 1-digit" bounds two operands, and only one had a key, 2026-08-19 — Unit 3 completion + full `run_all` verification, 2026-08-20 (tick 8) — A dispatch-only tick: coverage 52 → 127, 2026-08-23 — §6G: an attestation must show its work, 2. `§1A-reach` false failure — `mat_g3_na_q2_0` (money_peso), 3. KG monotonicity — `order_of_operations` introduced but not propagated (+18 more)
...
Cohesion: 0.10
Nodes (16): get_dnas_for_formatter(), get_variant_curriculum_gate(), is_compatible(), Any, Practice Generation — DNA-Formatter Compatibility Table ========================, Validate a lab UI selection and return compatibility info.      Args:         dn, Return the curriculum introduction point for a variant, or None if no gate., Return True if the formatter is compatible with the given DNA concept.      Args (+8 more)
...
Cohesion: 0.20
Nodes (11): generate_hints(), generate_params(), _inverse_op(), Any, DNA: Missing Number (Number & Algebra)  Covers MATATAG grades 1–3 missing-number, Rejection-sample a missing-number equation matching difficulty_profile.      Ret, RuntimeError, AuditHarnessError (+3 more)
...
Cohesion: 0.16
Nodes (16): get_node_info(), Return the full knowledge-graph node dict for a node.      Args:         node_id, capability_findings(), coverage(), _cputime_to_seconds(), _etime_to_seconds(), ledger_state(), main() (+8 more)

### Community 215 - "mastery_drill.py"
Cohesion: 0.11
Nodes (29): _apply(), _drift_attested_content(), _first_failure_line(), main(), _marker_present(), Mutation, _plant_contradicted_entry(), _plant_silent_substitution() (+21 more)
...
Cohesion: 0.17
Nodes (11): get_formatters_for_dna(), Return all formatter names compatible with a given DNA concept.      Args:, get_pipeline_status(), Return a health-check dict for the pipeline.      Checks:       - Whether each o, PracticeOrchestrator, Any, Bridge Layer separating the generation logic from the FastAPI routing/server lay, compute() (+3 more)

### Community 259 - "generate_params"
Cohesion: 0.10
Nodes (29): DimensionSpec, ErrorPattern, extract_continuous_scalar(), extract_discrete_level(), Practice Generation — DNA Base Definitions  All dataclasses, enums, and shared u, A pedagogically meaningful wrong answer.      formula:          SymPy expression, Extracts a discrete level from the difficulty profile.     If the value is a str, Extracts a continuous scalar from the difficulty profile.     Handles floats, di (+21 more)
...
### Community 267 - "mastery_drill.py"
Cohesion: 0.40
...
Cohesion: 0.61
Nodes (6): backoff_until_window_reopens(), log(), probe_window_open(), run_capped(), run_tick(), hardening_runner.sh script
...
### Community 272 - "test_answer_leak_and_reach.py"
Cohesion: 0.21
...
### Community 275 - "frontend_contract_auditor.py"
Cohesion: 0.26
...
### Community 276 - "fmt_error_detect.py"
Cohesion: 0.26
...
### Community 278 - "get_node_competency_bounds"
Cohesion: 0.24
...
### Community 284 - "CompetencyConfiguration"
Cohesion: 0.36
...
### Community 287 - "fmt_ruler_measure.py"
Cohesion: 0.33
...
### Community 316 - "_numeric_payload_values"
Cohesion: 0.33
...
### Community 319 - "_rationale_skeleton"
Cohesion: 0.33
...
### Community 322 - "rebuild_knowledge_graph.py"
Cohesion: 0.50
...
### Community 325 - "Tick 22 — 2026-08-22 — the range fix lands, and a third zero-factor path"
Cohesion: 0.50
...
### Community 326 - "_get_available_formats"
Cohesion: 0.67
Nodes (3): _get_available_formats(), get_matatag_nodes(), Return which formats are available for a node: ['mcq'], ['visual'], or ['mcq','v

## Knowledge Gaps
- **1202 isolated node(s):** `graphify`, `PackageDescription`, `Foundation`, `PathKit`, `graphify-mcp` (+1197 more)
...
- **Why does `FormattedProblem` connect `models.py` to `health`, `generate_params`, `Community 4`, `_get_available_formats`, `.axis_level_index`, `Community 8`, `fmt_array_grid.py`, `Community 138`, `Community 13`, `Community 18`, `frontend_contract_auditor.py`, `fmt_error_detect.py`, `fmt_ruler_measure.py`, `fmt_fill_in_table.py`, `Community 50`, `Community 51`, `1. Local Prototyping: Data Seeding`, `Community 59`, `Community 60`, `mastery_drill.py`, `get_gemini_models`, `Community 70`, `Community 77`, `Community 78`, `Community 79`, `Community 92`, `Community 93`, `Community 95`, `Community 99`, `Community 100`, `2026-08-13 — perimeter: an impossible triangle, and two shapes that never appeared`?**
  _High betweenness centrality (0.032) - this node is a cross-community bridge._
- **Why does `QuestionContext` connect `Community 50` to `generate_params`, `_get_available_formats`, `.axis_level_index`, `Community 138`, `Community 13`, `Community 18`, `fmt_error_detect.py`, `_combined_interests`, `fmt_ruler_measure.py`, `fmt_fill_in_table.py`, `Community 51`, `1. Local Prototyping: Data Seeding`, `Community 59`, `Community 60`, `get_gemini_models`, `Community 70`, `Community 77`, `Community 78`, `Community 79`, `Community 92`, `Community 93`, `models.py`, `Community 95`, `Community 99`, `Community 100`, `2026-08-13 — perimeter: an impossible triangle, and two shapes that never appeared`?**
  _High betweenness centrality (0.029) - this node is a cross-community bridge._
- **Why does `Phase F — full `run_all` verification and three more regressions it caught — 2026-08-02` connect `Community 103` to `Queries`, `2026-08-13 — length_measurement: the largest FAIL cluster in the tree`, `fmt_ruler_measure.py`, `generate_params`, `missing_number.py`, `2026-08-19 — Hardening Unit 3: mutation coverage for §5 and §6`, `2026-08-21 (tick 17) — §2B was guarding one direction only`, `SKILL.md`, `2026-08-20 — Tick C: `mat_g2_mg_q4_3` content defects fixed at root, and `explain` built`, `2026-08-14 — "2-digit by 1-digit" bounds two operands, and only one had a key`, `2026-08-19 — Hardening Unit 3: mutation coverage for §5 and §6`, `_normalize_stem`, `2026-08-14 — The blank landed on a given, not on the unknown`, `test_planted_mutation_bounds_only_provider_is_caught_by_name`, `test_attestation_verdict_must_be_binary`, `Community 36`, `run_batch`, `test_generic_textual_formatter_is_not_a_provider`, `generate_params`, `SKILL.md`, `2026-08-20 (tick 8) — A dispatch-only tick: coverage 52 → 127`, `get_node_competency_bounds`, `matatag_loader.py`, `probability_language.py`, `validate_capability.py`, `fractions.py`, `2026-08-23 — the mutation harness could not prove the three checks that make a new grade safe`, `Tick 21 — 2026-08-22 — the candidate pool was quadratic in a curriculum ceiling`, `fmt_array_grid.py`, `2026-08-20 — §5 worker death: a silent unbounded wait in the last gate`, `Tick 22 — 2026-08-22 — the range fix lands, and a third zero-factor path`, `fmt_ordering.py`, `2026-08-12 — Tick C cluster 1c: two repeated-addition competencies bound identically`, `get_interest_themes`, `test_semantic_leak_guards.py`, `test_semantic_leak_guards.py`, `run_audit`, `get_interest_themes`, `generate_params`, `models.py`, `get_node_competency_bounds`, `2026-08-14 — The blank landed on a given, not on the unknown`, `2026-08-12 — Tick C cluster 1: mass/capacity unit axis never bound (6 nodes)`, `RedisDict`, `2026-08-12 — Tick C cluster 1d: array formatters unreachable on the node that names arrays`, `2026-08-13 — The g3_mg_q1 area cluster: four siblings, one unbound key (Tick C)`, `health`, `get_engine`, `validate_compat.py`, `_gen_g2_na_q1_numbers`, `_combined_interests`, `Ground Rule 2: Spec Corrections & Baseline Fixes`, `2026-08-13 — The tiling word problem had no determinate answer (Tick C)`, `symmetry_slides.py`, `Anti-Patterns`, `2026-08-20 — §6F: an Attester verdict that nothing enforces is not a check`, `Part 3 — Concrete restructuring of the current docs/`, `2026-08-19 — Hardening Unit 1: restore two weakened tests (deliberate documented red)`, `2026-08-14 — A stated width is a floor as well as a ceiling`, `Community 127`?**
