...
## Corpus Check
- 869 files · ~1,643,789 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5843 nodes · 9276 edges · 425 communities (386 shown, 39 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 138 edges (avg confidence: 0.54)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `b2a05599`
...
## Communities (425 total, 39 thin omitted)
...
Cohesion: 0.12
Nodes (26): Translate a FormattedProblem to the legacy skeleton dict format.      This shim, to_legacy_dict(), log_scale_value(), The integer a 0-1 position `t` maps to on a logarithmic axis [min_val, max_val]., get_matatag_lab_config(), Return full lab configuration for a MATATAG node.      Includes:     - difficult, _audit_node(), build_test_profiles() (+18 more)
...
Cohesion: 0.16
Nodes (13): _contains_forbidden_term(), get_interest_emoji(), get_interest_slots(), pick_interest(), Random, Practice Generation — Interest Bank =====================================  Loads, Build a single-problem slot dict for the given interest theme.      Randomly sel, Return the declared learner-visible emoji for a supported interest.      Unlike (+5 more)
...
Cohesion: 0.05
Nodes (62): get_supported_variants(), Return variants supported by a specific DNA + formatter combination.      Applie, Any, Registry to map visual types to their Pydantic schemas., VisualSchemaRegistry, _answer_leaks_into_stem(), applicability_failures(), count_addition_carries() (+54 more)
...
Cohesion: 0.04
Nodes (52): 1. Distractor Non-Determinism Fix in `place_value.py`, 1. Fractions answer-key false positive — `mat_g2_na_q4_0`/`_1`/`_2`, 2026-08-13 — The G1 measure items: a visual for the task, and a size model for the units, 2026-08-19 — Unit 3 completion + full `run_all` verification, 2026-08-20 — §5 worker death: a silent unbounded wait in the last gate, 2026-08-20 (tick 6) — There is no pytest deadlock, and three tests I broke in tick 3, 2026-08-21 (tick 14) — CORRECTION: §2B's 236 was two defects, not one, 2026-08-23 — §6G: an attestation must show its work (+44 more)
...
Cohesion: 0.18
Nodes (10): get_dnas_for_formatter(), Return all DNA concept names that support a given formatter.      Performs a rev, enumerate_obligations(), Obligation, The student-path obligation manifest (plan step 0B, `H-04`).  WHAT AN OBLIGATION, Every reachable (node, DNA, formatter, discrete assignment), and every rejection, Rejection, The obligation manifest (§11, H-04): determinism, and the model it claims.  `H-0 (+2 more)
...
Cohesion: 0.05
Nodes (57): get_grade_appropriate_interests(), Return all interest IDs whose grade_band covers the given grade.      An interes, get_all_node_ids(), get_node_competency_bounds(), get_node_formatters(), get_node_info(), Get competency-specific bounds for a node's difficulty dimensions.          Retu, Return the union of compatible formatters from all DNAs for a node.      Looks u (+49 more)
...
Cohesion: 0.11
Nodes (17): get_node_dnas(), Return the list of DNA concept names for a node.      Args:         node_id: MAT, compute(), main(), Returns (exclusions, broken) -- broken = pairs that raise a NON-eligibility erro, _continuous_axes_with_default_range(), Tests for the axes-catalog logarithmic scale and the auditor's ``build_test_prof, Yield (concept, axis) for every continuous axis that has both     default_min an (+9 more)
...
Cohesion: 0.06
Nodes (35): count_noun(), Any, A reusable story template with named slots.      Slots are filled from the stude, Fill template with interest slots and numeric values.                  Handles s, The count noun to write after `count`. Singular when the count is exactly 1., The singular of a count-noun phrase. Returns `phrase` unchanged when already sin, Serialize to dict for API response., Spine (+27 more)
...
## Knowledge Gaps
- **2063 isolated node(s):** `graphify`, `PackageDescription`, `Foundation`, `PathKit`, `graphify-mcp` (+2058 more)
...
- **Why does `FormattedProblem` connect `test_omission_survives_provenance_but_not_coverage` to `Queries`, `2026-08-14 — "2-digit by 1-digit" bounds two operands, and only one had a key`, `Community 4`, `fmt_array_grid.py`, `test_bounds_length_is_never_the_discriminator`, `test_contradicted_entry_is_caught_by_name`, `Community 13`, `Mutations`, `run_all.py`, `2026-08-23 — tick 24 — 90 clauses attested; the medium clauses are where the tree is thinnest`, `Community 50`, `Community 51`, `Spine`, `Community 59`, `Community 60`, `fractions.py`, `get_gemini_models`, `Community 70`, `Community 75`, `Community 79`, `Community 92`, `Community 94`, `Tick 23 — 2026-08-22 — the contradiction closes, and the Attester's aside was the next bug`, `_numeric_payload_values`, `Any`, `fmt_ruler_measure.py`, `_profile_echo_keys`?**
...
- **Why does `QuestionContext` connect `Community 50` to `Queries`, `fmt_array_grid.py`, `test_bounds_length_is_never_the_discriminator`, `test_contradicted_entry_is_caught_by_name`, `Community 13`, `test_omission_survives_provenance_but_not_coverage`, `Mutations`, `run_all.py`, `Community 51`, `Spine`, `Part 1 — Extending the pipeline`, `Community 59`, `Community 60`, `fractions.py`, `get_gemini_models`, `Community 70`, `Community 79`, `Community 92`, `Community 94`, `Tick 23 — 2026-08-22 — the contradiction closes, and the Attester's aside was the next bug`, `_numeric_payload_values`, `Any`, `fmt_ruler_measure.py`, `_profile_echo_keys`?**
