...
## Corpus Check
- 874 files · ~1,720,281 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5959 nodes · 9474 edges · 449 communities (404 shown, 45 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 142 edges (avg confidence: 0.54)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `11d14915`
...
## Communities (449 total, 45 thin omitted)
...
Cohesion: 0.07
Nodes (36): get_formatters_for_dna(), Return all formatter names compatible with a given DNA concept.      Args:, RuntimeError, _audit_node(), AuditHarnessError, build_test_profiles(), _check_cap_vs_formatter_code(), check_checklist_compliance() (+28 more)
...
Cohesion: 0.05
Nodes (60): log_scale_value(), The integer a 0-1 position `t` maps to on a logarithmic axis [min_val, max_val]., get_supported_variants(), Return variants supported by a specific DNA + formatter combination.      Applie, Any, Registry to map visual types to their Pydantic schemas., VisualSchemaRegistry, _answer_leaks_into_stem() (+52 more)
...
Cohesion: 0.08
Nodes (38): count_noun(), The count noun to write after `count`. Singular when the count is exactly 1., decompose_to_places(), "N is H hundreds, T tens, and O ones." (place values whose digit is 0     are om, generate_params(), Generate (a, b) with a >= b that satisfy the difficulty_profile constraints., _build_equation_sentence(), format_cloze() (+30 more)
...
Cohesion: 0.07
Nodes (55): get_db(), FastAPI dependency that yields a database session.     Guarantees session closur, SessionLocal(), _combined_interests(), health(), Merge parent-set interest_tags and student-set student_interest_tags into a, Background task to pre-generate questions into the cache.     Uses parallel exec, Liveness probe. Reaching this proves the whole module graph imported and     uvi (+47 more)
...
Cohesion: 0.06
Nodes (50): node_grade_quarter(), The (grade, quarter) a node sits at, parsed from its id.      One copy, because, get_node_competency_bounds(), get_node_dnas(), get_node_formatters(), Get competency-specific bounds for a node's difficulty dimensions.          Retu, Return the list of DNA concept names for a node.      Args:         node_id: MAT, Return the union of compatible formatters from all DNAs for a node.      Looks u (+42 more)
...
Cohesion: 0.07
Nodes (50): _answer_display(), _answer_value(), _node_file(), _normalize(), _option_values(), _packet_core_from_review(), _provenance_corpus(), Any (+42 more)
...
Cohesion: 0.16
Nodes (12): Translate a FormattedProblem to the legacy skeleton dict format.      This shim, to_legacy_dict(), PracticeOrchestrator, Any, Bridge Layer separating the generation logic from the FastAPI routing/server lay, is_prompt_target_formatter(), _is_rendered_leak(), _no_rendered_leaks() (+4 more)
...
Cohesion: 0.14
Nodes (14): 2026-09-19 — H-06 / M2 step 6: four capability gaps closed, one needing a harness rule change, §2B caught the second-order effect, and it was regenerated rather than hand-edited, A FIX AT THE DATA LAYER GATES NOTHING — what is still unguarded, Counting `direction`: fixed, WITH the harness rule it required — owner-approved, counting `direction`, needed a coordinated harness change and has its own section), Definition of Done, Fixed, each measured before and after on the default student path (three below; the fourth,, FIXED: the `estimate` task keyed a ZERO measurement (+6 more)
...
### Community 361 - "Any"
Cohesion: 0.17
Nodes (12): get_parent_config(), get_parent_graph(), parent_login(), Any, Session, Parent Login. Auto-registers alphanumeric password on first run for developer co, Returns highly granular parallel linear tracks and their exact masteries     mat, Fetch global parent portal settings. (+4 more)
...
Cohesion: 0.22
Nodes (8): Claim your row before you start, FIRST: establish whether the tree is mid-re-proof, Handoff — continue the Phase 2 hardening plan, Read this, in this order, Rules this handoff will not let you skip, The four things worth saying twice, Two open decisions that are the owner's, not yours, What is left
...
Cohesion: 0.10
Nodes (28): is_variant_available_at(), Check if a variant is available at a specific grade/quarter per curriculum., A reusable story template with named slots.      Slots are filled from the stude, Spine, _build_symbolic_question(), _detect_axes_served(), _eval_error_formula(), generate_context() (+20 more)
...
Cohesion: 0.07
Nodes (52): max_regrouping_places(), Largest borrow count achievable for any (a, b) with a <= max_minuend, a >= b., get_pipeline_status(), Return a health-check dict for the pipeline.      Checks:       - Whether each o, _bound_allows(), build_group(), build_packet(), _continuous_axis_names() (+44 more)
...
Cohesion: 0.22
Nodes (12): Collapse a rationale to the structure that survives per-node substitution., _rationale_skeleton(), _blindness(), build_records(), _load(), main(), Any, Attester filing — turn one blind Attester's verdicts into §6F/§6G-valid records. (+4 more)
...
Cohesion: 0.12
Nodes (14): get_variant_curriculum_gate(), is_compatible(), Any, Practice Generation — DNA-Formatter Compatibility Table ========================, Validate a lab UI selection and return compatibility info.      Args:         dn, Return the curriculum introduction point for a variant, or None if no gate., Return True if the formatter is compatible with the given DNA concept.      Args, validate_lab_selection() (+6 more)
...
### Community 418 - "2026-08-14 — "2-digit by 1-digit" bounds two operands, and only one had a key"
Cohesion: 0.50
...
### Community 428 - "_combined_interests"
Cohesion: 0.36
...
### Community 445 - "2026-08-13 — length_measurement: the largest FAIL cluster in the tree"
Cohesion: 0.50
...
### Community 446 - "2026-08-20 (tick 7) — The harness now runs its own tests"
Cohesion: 0.50
Nodes (4): 2026-08-20 (tick 7) — The harness now runs its own tests, Proved, not assumed, What shipped, Why

### Community 447 - "2026-08-28 — D0 (§6H) and the first D1 blind re-review batch"
Cohesion: 0.50
...
## Knowledge Gaps
- **2105 isolated node(s):** `graphify`, `PackageDescription`, `Foundation`, `PathKit`, `graphify-mcp` (+2100 more)
