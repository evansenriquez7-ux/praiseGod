# Graph Report - ccmed  (2026-09-11)

## Corpus Check
- 666 files · ~1,281,768 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 4980 nodes · 7547 edges · 408 communities (363 shown, 45 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 104 edges (avg confidence: 0.57)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `950bc9a8`
...
- [[_COMMUNITY_get_interest_themes|get_interest_themes]]
- [[_COMMUNITY__plant_inverted_bound|_plant_inverted_bound]]
...
- [[_COMMUNITY_generate_params|generate_params]]
- [[_COMMUNITY_2026-08-14 — 2-digit by 1-digit bounds two operands, and only one had a key|2026-08-14 — "2-digit by 1-digit" bounds two operands, and only one had a key]]
- [[_COMMUNITY__plant_fabricated_quote|_plant_fabricated_quote]]
- [[_COMMUNITY_generate_params|generate_params]]
- [[_COMMUNITY__plant_single_reviewer_identity|_plant_single_reviewer_identity]]
- [[_COMMUNITY__plant_template_rationale|_plant_template_rationale]]
- [[_COMMUNITY_2026-08-20 (tick 7) — The harness now runs its own tests|2026-08-20 (tick 7) — The harness now runs its own tests]]
...
3. `Phase F — full `run_all` verification and three more regressions it caught — 2026-08-02` - 95 edges
4. `QuestionContext` - 86 edges
5. `FormattedProblem` - 79 edges
6. `DNA` - 57 edges
7. `augment_distractors()` - 50 edges
8. `get_all_node_ids()` - 45 edges
9. `shuffle_options()` - 42 edges
10. `VocabGated` - 37 edges
...
## Communities (408 total, 45 thin omitted)
...
Cohesion: 0.07
Nodes (38): get_supported_variants(), Any, Return variants supported by a specific DNA + formatter combination.      Applie, Validate a lab UI selection and return compatibility info.      Args:         dn, validate_lab_selection(), _audit_node(), AuditHarnessError, build_test_profiles() (+30 more)
...
Cohesion: 0.09
Nodes (21): get_formatters_for_dna(), Return all formatter names compatible with a given DNA concept.      Args:, _get_available_formats(), get_intro_status(), get_matatag_lab_interests(), get_matatag_nodes(), get_matatag_progress(), get_node_config() (+13 more)
...
Cohesion: 0.08
Nodes (35): Practice Generation — Validation Manifest  Central source of truth for the valid, Every §-ref registered to `phase`., refs_in_phase(), _parse_contract_section_refs(), Practice Generation — Validation Harness Runner  Runs the validators in the prac, Run the fast unit suite and report it as a harness stage (§0).      Subprocess r, Run the harness. `phase=1` runs only the artifact-free band, `phase=2` only the, Extract every '§1A'-style reference from docs/pgen_contract.md's rule     table (+27 more)
...
Cohesion: 0.13
Nodes (33): get_db(), FastAPI dependency that yields a database session.     Guarantees session closur, _combined_interests(), health(), Merge parent-set interest_tags and student-set student_interest_tags into a, Liveness probe. Reaching this proves the whole module graph imported and     uvi, get_axes_for_concept(), Practice Generation — Difficulty Axes Catalog ================================== (+25 more)
...
Cohesion: 0.11
Nodes (17): 1. Pipeline Layout & Architecture, 2. Core Metamorphic Checkpoints, 3. CLI Execution Reference Table, 4. Diagnostics & Troubleshooting Traps, 5. The Harness As It Actually Is (regenerate this when checks change), A. Strict Scalar Mapping (Boundary Checks), B. Metamorphic Sensitivity Checks, C. Semantic Leak Safeguards (+9 more)
...
Cohesion: 0.08
Nodes (25): _drift_attested_content(), _plant_attester_without_evidence(), _plant_contradicted_entry(), _plant_incomplete_review(), _plant_inverted_bound(), _plant_orphan_provider(), _plant_silent_substitution(), _plant_single_attester_identity() (+17 more)

### Community 94 - "Community 94"
Cohesion: 0.13
Nodes (21): _answer_leaks_into_stem(), get_variant_combinations(), _normalize_stem(), _option_in_stem(), Any, Lowercase the stem and render LaTeX fractions as 'a/b' so an option like     '1/, Whether an option value appears in the stem as a *whole* token.      Substring m, §1F's second, independent path: the stem says the answer in words.      The orig (+13 more)
...
Cohesion: 0.05
Nodes (42): 1. Distractor Non-Determinism Fix in `place_value.py`, 1. Fractions answer-key false positive — `mat_g2_na_q4_0`/`_1`/`_2`, 2026-08-13 — The unanswerable measure item was a routing bug, not a wording one, 2026-08-19 — Unit 3 completion + full `run_all` verification, 2026-08-20 (tick 6) — There is no pytest deadlock, and three tests I broke in tick 3, 2026-08-23 — §6G: an attestation must show its work, 2026-08-24 — tick 25 — 177 clauses attested; §6F UNATTESTED backlog reduced from 538 to 361, 2026-08-24 — tick 26 — 361 clauses attested across 89 nodes; §6F UNATTESTED backlog completely cleared (361 -> 0) (+34 more)

### Community 104 - "Community 104"
Cohesion: 0.06
Nodes (45): DNA, ErrorPattern, extract_continuous_scalar(), extract_discrete_level(), Practice Generation — DNA Base Definitions  All dataclasses, enums, and shared u, A pedagogically meaningful wrong answer.      formula:          SymPy expression, A mathematical term fragment gated behind vocab knowledge.      preferred:    Te, Specification of a mathematical concept for practice generation.      dna_type: (+37 more)
...
Cohesion: 0.16
Nodes (17): matatag_lab_submit(), Grade a MATATAG Lab answer.  Handles both visual and MCQ skeleton types.      Re, answers_match(), Any, Compare a submitted answer to the correct one by VALUE, for any non-MCQ format., test_answers_match.py ===================== Pins `backend.app.services.scoring.a, These are ordering answers -- a set comparison would accept a wrong sequence., REGRESSION GUARD. Parsing digits out of both sides ([9, 35] == [9, 35]) would ma (+9 more)
...
Cohesion: 0.20
Nodes (13): get_node_competency_bounds(), Get competency-specific bounds for a node's difficulty dimensions.          Retu, §2D — no configuration reachable through the serving API may serve content outsi, validate_config_respects_competency(), Planted mutation: an entry whose ONLY possible carrier is the shared catch-all., test_planted_mutation_bounds_only_provider_is_caught_by_name(), _formatters(), test_emoji_formatter_range_gate.py ================================== Emoji-base (+5 more)
...
Cohesion: 0.06
Nodes (58): max_regrouping_places(), Largest borrow count achievable for any (a, b) with a <= max_minuend, a >= b., _bound_allows(), build_group(), build_packet(), _main(), _max_difficulty_profile(), _max_regrouping_places_for() (+50 more)
...
Cohesion: 0.10
Nodes (29): option_rng(), Any, Random, Where the correct option sits must not be predictable.  Why this exists --------, A shuffle stream unique to this (node, seed), stable across processes., Shuffle in place using a (node, seed)-keyed stream, and return the list.      Ca, shuffle_options(), _build_pure_question() (+21 more)
...
Cohesion: 0.07
Nodes (23): get_node_formatters(), Return the union of compatible formatters from all DNAs for a node.      Looks u, §2C — a formatter a node advertises must be REACHABLE by the path students take., validate_advertised_formatters_are_reachable(), PracticeOrchestrator, Bridge Layer separating the generation logic from the FastAPI routing/server lay, compute(), main() (+15 more)
...
Cohesion: 0.08
Nodes (24): DimensionSpec, interpolate(), Any, Compute the dimension value at difficulty scalar t.          If override_min/ove, Serialize to dict for API response., Interpolate between min_val and max_val using the given scale type.      scale_t, Specification for a single difficulty dimension.      Defines how a dimension sc, generate_hints() (+16 more)
...
Cohesion: 0.14
Nodes (24): _answer_display(), _answer_value(), _node_file(), _normalize(), _option_values(), _provenance_corpus(), Any, Path (+16 more)
...
Cohesion: 0.11
Nodes (28): get_all_node_ids(), Return all node IDs from NODE_TO_DNA, optionally filtered.      Nodes are return, count_mutations(), count_nodes(), count_unit_tests(), count_variant_candidates(), main(), §7 — the suite's own census has not silently got smaller.  Why this exists ----- (+20 more)
...
Cohesion: 0.15
Nodes (17): _are_values_equal(), _eval_formula(), Any, Validate a formula-type DNA for structural correctness.      Checks:       1. an, Validate a visual_read-type DNA.      Checks:       1. answer_formula is None., Validate a static_bank-type DNA.      Checks:       1. answer_formula is None., Validate an algorithmic-type DNA for structural correctness.     Similar to vali, Import and validate all 27 DNA instances.      Dispatches each DNA to the approp (+9 more)
...
Cohesion: 0.15
Nodes (18): linear_interpolate(), Linear interpolation between min_val and max_val at position t., _assert_cases_determine(), _estimation_distractors(), generate_hints(), generate_params(), Any, Random (+10 more)
...
Cohesion: 0.05
Nodes (47): FormattedProblem, Final output of the practice generation pipeline.      Produced by a formatter o, `is_visual` is DERIVED, never asserted by a formatter.          It answers one q, decompose_to_places(), "N is H hundreds, T tens, and O ones." (place values whose digit is 0     are om, Practice Generation — Hint-Gated Experience Wrapper  Hints unlock one at a time., Increment hints_revealed by 1 and return the updated problem.      If the proble, reveal_next_hint() (+39 more)
...
Cohesion: 0.11
Nodes (21): Check if a pair satisfies regrouping difficulty based on COUNT of places.      C, True if `level` can be satisfied by some (a, b) with a + b <= max_result., regrouping_is_feasible(), _satisfies_regrouping(), _axis_scale(), _map_scalar_to_range(), Map a [0,1] scalar to the concrete value the orchestrator would     produce for, Return the scale ('logarithmic'/'linear'/None) the orchestrator uses     for `ax (+13 more)

### Community 274 - "2026-08-21 (tick 17) — §2B was guarding one direction only"
Cohesion: 0.12
Nodes (26): AST, check_phase_registry_failures(), declared_assertion_labels(), harness_assertion_labels(), _literal_prefix(), main(), matrix_assertion_labels(), printed_check_labels() (+18 more)
...
Cohesion: 0.24
Nodes (11): _clear_marker(), _install_kill_safety(), _invert_number_line_axis(), main(), _plant_unsanctioned_requires_ignore(), Mutation harness — verify the verifier (pgen_hardening.md Phase 4).  The validat, Swap start/end in every number-line payload the formatter builds.      A find/re, Drop a competency word into `requires_ignore` without sanctioning it in the lock (+3 more)
...
Cohesion: 0.18
Nodes (14): _apply(), _first_failure_line(), _marker_present(), Mutation, Apply every edit, returning original contents for restoration., Persist what is planted, so a kill -9 is still recoverable., A plain substring, or -- with ' && ' -- several substrings on the SAME line., Apply, run, restore. Returns (detected, evidence-line).      Detection means the (+6 more)
...
Cohesion: 0.10
Nodes (23): EngineProxy, get_engine(), SessionLocal(), Background task to pre-generate questions into the cache.     Uses parallel exec, replenish_question_cache(), Background task to pre-generate questions into the cache.     Uses parallel exec, replenish_question_cache(), Background task to pre-generate questions into the cache.     Uses parallel exec (+15 more)
...
Cohesion: 0.25
Nodes (8): _fresh_samples(), _plant_review_drops_options(), _plant_stale_answer_same_key(), _plant_stale_review(), Every (path, data, index, sample, current_render) whose recorded stem STILL matc, Rewrite one CURRENTLY-FRESH sample's recorded stem so the live render no longer, Move the correct value onto a distractor THAT WAS ALREADY OFFERED, keeping the s, Delete the recorded `options` from a sample whose live render still offers some.
...
### Community 368 - "generate_params"
Cohesion: 0.29
Nodes (8): _attestation_record_for(), _plant_attestation_answer_drift(), _plant_attestation_option_drift(), The live record whose packet is for `node_id`, plus its parsed data., Assert a fixture sample still renders what its record says, and return the rende, Move the answer an attestation was filed against, leaving its stem byte-identica, Two branches of §6F's option comparison, planted together so one run says whethe, _require_fresh_sample()

### Community 369 - "generate_params"
Cohesion: 0.40
Nodes (5): Practice Generation — Interest Invariance Validation  Verifies that interest-the, Verify that interest theme does not alter the correct answer.      For each of `, Run interest invariance checks for all requires_context=True formula DNAs., validate_all_interest_invariance(), validate_interest_invariance()
...
Cohesion: 0.50
Nodes (4): generate_hints(), generate_params(), Any, Sample one scenario from the static pool filtered by difficulty profile.

### Community 374 - "generate_params"
Cohesion: 0.50
...
### Community 376 - "2026-08-20 (tick 8) — A dispatch-only tick: coverage 52 → 127"
Cohesion: 0.50
...
### Community 378 - "2026-08-20 (tick 7) — The harness now runs its own tests"
Cohesion: 0.50
...
### Community 381 - "get_interest_themes"
Cohesion: 0.25
...
Cohesion: 0.50
Nodes (4): _declared_nodes(), ((node_id, competency, requires, requires_ignore) per well-formed node, guard er, _plant_withdrawn_attestation(), Delete one (node, capability) verdict from EVERY record that carries it.      §6

### Community 394 - "2026-08-14 — "2-digit by 1-digit" bounds two operands, and only one had a key"
Cohesion: 0.50
...
- **Why does `FormattedProblem` connect `fmt_numeric_input.py` to `2026-08-14 — "2-digit by 1-digit" bounds two operands, and only one had a key`, `fmt_array_grid.py`, `Community 138`, `Community 13`, `Community 18`, `test_omission_survives_provenance_but_not_coverage`, `_combined_interests`, `run_all.py`, `2026-08-20 (tick 7) — The harness now runs its own tests`, `test_planted_mutation_bounds_only_provider_is_caught_by_name`, `Community 50`, `Community 51`, `Spine`, `Part 1 — Extending the pipeline`, `Community 59`, `Community 60`, `fractions.py`, `get_gemini_models`, `Community 70`, `2026-08-13 — perimeter: an impossible triangle, and two shapes that never appeared`, `Community 75`, `Community 77`, `Community 79`, `Community 92`, `Community 95`, `get_all_node_ids`, `Community 104`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
- **Why does `QuestionContext` connect `Community 50` to `fmt_numeric_input.py`, `fmt_array_grid.py`, `Community 138`, `Community 11`, `Community 13`, `Community 18`, `test_omission_survives_provenance_but_not_coverage`, `run_all.py`, `2026-08-20 (tick 7) — The harness now runs its own tests`, `test_planted_mutation_bounds_only_provider_is_caught_by_name`, `Community 51`, `Spine`, `Part 1 — Extending the pipeline`, `Community 59`, `Community 60`, `fractions.py`, `get_gemini_models`, `Community 70`, `Community 77`, `Community 79`, `Community 92`, `Community 95`, `Community 104`?**
  _High betweenness centrality (0.027) - this node is a cross-community bridge._
- **Why does `get_all_node_ids()` connect `RedisDict` to `socratic_chat_exchange`, `Community 162`, `Community 99`, `CompetencyConfiguration`, `Community 4`, `2026-08-14 — "2-digit by 1-digit" bounds two operands, and only one had a key`, `.axis_level_index`, `Community 41`, `Community 11`, `Community 16`, `test_attestation_without_samples_cannot_be_checked_and_fails`, `hardening_supervisor.py`, `1. Local Prototyping: Data Seeding`, `mass_capacity.py`, `fmt_numeric_input.py`?**
