...
## Corpus Check
- 754 files · ~1,509,363 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 5132 nodes · 7856 edges · 405 communities (356 shown, 49 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 109 edges (avg confidence: 0.57)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `c007b8ae`
...
## Communities (405 total, 49 thin omitted)
...
Cohesion: 0.09
Nodes (29): get_node(), Return the raw knowledge-graph node dict for node_id, or None.      Args:, load_dna(), Practice Generation — Validation Manifest  Central source of truth for the valid, Import the DNA module and return its DNA instance.     Raises ImportError if loa, Every §-ref registered to `phase`., refs_in_phase(), _parse_contract_section_refs() (+21 more)
...
Cohesion: 0.06
Nodes (57): LearningDomain, Any, Enforces Grade-Level Vocabulary Constraints (Community 2), Extracts common misconceptions (Community 5), Base class for Domain Services.     Encapsulates DNA generation logic, curriculu, BalanceScaleParams, BarChartParams, BaseVisualParams (+49 more)
...
Cohesion: 0.06
Nodes (55): Attempt, CompetencyConfiguration, MasteryState, NodeIntroView, ParentAccount, QuestionFlag, SkillEdge, SkillNode (+47 more)
...
Cohesion: 0.11
Nodes (27): applicability_failures(), count_addition_carries(), count_subtraction_borrows(), coverage_regressions(), expected_checks_for_node(), formatter_refused_at_node(), get_expected_mapped_value(), main() (+19 more)
...
Cohesion: 0.13
Nodes (32): get_db(), FastAPI dependency that yields a database session.     Guarantees session closur, SessionLocal(), Background task to pre-generate questions into the cache.     Uses parallel exec, replenish_question_cache(), get_axes_for_concept(), Practice Generation — Difficulty Axes Catalog ==================================, Return the UI-ready axis list for a concept, or [] if not found. (+24 more)
...
Cohesion: 0.04
Nodes (47): 1. Distractor Non-Determinism Fix in `place_value.py`, 1. Fractions answer-key false positive — `mat_g2_na_q4_0`/`_1`/`_2`, 2026-08-13 — Finishing the pairing fix: the unit, not the floor, 2026-08-13 — length_measurement: the largest FAIL cluster in the tree, 2026-08-13 — The unanswerable measure item was a routing bug, not a wording one, 2026-08-19 — Unit 3 completion + full `run_all` verification, 2026-08-20 — §5 worker death: a silent unbounded wait in the last gate, 2026-08-23 — §6G: an attestation must show its work (+39 more)
...
Cohesion: 0.07
Nodes (50): _answer_display(), _answer_value(), _node_file(), _normalize(), _option_values(), _provenance_corpus(), Any, Path (+42 more)
...
Cohesion: 0.40
Nodes (5): Session, Initiates a telemetry tracking session for window defense telemetry., Updates statistics in the telemetry logs database., start_telemetry_session(), update_telemetry_session()
...
Cohesion: 0.04
Nodes (86): max_regrouping_places(), Largest borrow count achievable for any (a, b) with a <= max_minuend, a >= b., find_node_id(), get_all_node_ids(), get_node_competency_bounds(), get_node_dnas(), get_node_formatters(), get_node_info() (+78 more)
...
Cohesion: 0.15
Nodes (13): 0. Establish a reproducible baseline and complete migration inventory, 0A. Make Phase 1 hermetic, complete, and honest before repairing content, 0B. Build and budget the student-path obligation manifest, 1. Define the merged evidence record and lossless filing path, 2. Unify rendering, sample coverage, and freshness before retiring packets, 3. Integrate and finish the existing §1J/§1K bounded lints, 3A. Build contextual logic as a separately staged capability, 3B. Generalize the clause→payload rule table (+5 more)
...
Cohesion: 0.15
Nodes (13): 6. Fix all content by root cause, including required capabilities, 7. Review stable content in one programme, 8. Final verification and evidence filing, 8A. Retire the two `slow` audits as gates, keeping the module, Evidence and current implementation state, Evidence for preparation and implementation-state review, Implementation file map, M2 — Resolve the content and earn current reviews (+5 more)
...
Cohesion: 0.12
Nodes (12): is_variant_supported(), Check if a specific variant value is supported for a DNA + formatter.      Args:, test_formatter_supports_profile.py =================================== Regressio, The orchestrator must set problem.dna_name to the actually-chosen DNA., The cross-DNA formatter filter, asserted as an INVARIANT over every case where, The complementary case: when the formatter IS offered by the node's own DNA,, A competency can bind an axis to a SET of literal values, not just one. 78 such, ALL, not ANY. The DNA picks one member per seed, so a formatter is eligible (+4 more)
...
Cohesion: 0.08
Nodes (34): count_noun(), Any, Compute the dimension value at difficulty scalar t.          If override_min/ove, The count noun to write after `count`. Singular when the count is exactly 1., Serialize to dict for API response., decompose_to_places(), "N is H hundreds, T tens, and O ones." (place values whose digit is 0     are om, generate_hints() (+26 more)
...
Cohesion: 0.10
Nodes (24): Generate a single practice problem and return it as a dict.      This is the sin, run(), matatag_lab_v2_generate(), Generate a practice problem using the new practice_gen pipeline., Exception, build(), main(), Any (+16 more)
...
Cohesion: 0.50
Nodes (4): Random, Practice Generation — Story Spines ====================================  Defines, Choose the best narrative Spine for the current problem context.      Eligibilit, select_spine()
...
Cohesion: 0.15
Nodes (14): get_variant_combinations(), _numeric_payload_values(), _profile_echo_keys(), Any, Every `given_values` key that is an echo of the request rather than generated, Every plain number a generated problem exposes to the student, labelled for, §1F's second, independent path: the stem says the answer in words.      The orig, Score the complexity of whole numbers/fractions in problem.given_values. (+6 more)
...
Cohesion: 0.67
Nodes (3): compute(), main(), Returns (exclusions, broken) -- broken = pairs that raise a NON-eligibility erro
...
Cohesion: 0.14
Nodes (15): A reusable story template with named slots.      Slots are filled from the stude, Fill template with interest slots and numeric values.                  Handles s, The singular of a count-noun phrase. Returns `phrase` unchanged when already sin, Spine, to_singular_phrase(), collect_findings(), main(), Any (+7 more)
...
## Knowledge Gaps
- **1986 isolated node(s):** `graphify`, `PackageDescription`, `Foundation`, `PathKit`, `graphify-mcp` (+1981 more)
...
- **Why does `FormattedProblem` connect `test_omission_survives_provenance_but_not_coverage` to `2026-08-14 — "2-digit by 1-digit" bounds two operands, and only one had a key`, `Community 4`, `fmt_numeric_input.py`, `fmt_array_grid.py`, `Community 138`, `Community 13`, `Community 18`, `_combined_interests`, `Mutations`, `run_all.py`, `Community 25`, `2026-08-20 (tick 7) — The harness now runs its own tests`, `test_planted_mutation_bounds_only_provider_is_caught_by_name`, `Community 50`, `Community 51`, `Spine`, `Part 1 — Extending the pipeline`, `Community 59`, `Community 60`, `fractions.py`, `get_gemini_models`, `Community 70`, `Community 75`, `Community 77`, `Community 79`, `Community 92`, `Community 95`, `Community 104`?**
  _High betweenness centrality (0.027) - this node is a cross-community bridge._
- **Why does `get_node_info()` connect `1. Local Prototyping: Data Seeding` to `socratic_chat_exchange`, `Community 99`, `Community 4`, `Community 41`, `Community 11`, `Community 75`, `Community 16`, `test_omission_survives_provenance_but_not_coverage`, `Community 117`, `hardening_supervisor.py`, `Community 93`?**
