...
## Corpus Check
- 499 files · ~822,004 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3728 nodes · 5878 edges · 302 communities (283 shown, 19 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 91 edges (avg confidence: 0.57)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `91d631aa`
...
3. `Phase F — full `run_all` verification and three more regressions it caught — 2026-08-02` - 72 edges
4. `DNA` - 57 edges
...
Cohesion: 0.08
Nodes (36): get_formatters_for_dna(), Return all formatter names compatible with a given DNA concept.      Args:, get_matatag_lab_config(), Return full lab configuration for a MATATAG node.      Includes:     - difficult, _audit_node(), AuditHarnessError, build_test_profiles(), _check_cap_vs_formatter_code() (+28 more)
...
Cohesion: 0.12
Nodes (36): _combined_interests(), health(), Merge parent-set interest_tags and student-set student_interest_tags into a, Liveness probe. Reaching this proves the whole module graph imported and     uvi, get_axes_for_concept(), Practice Generation — Difficulty Axes Catalog ==================================, Return the UI-ready axis list for a concept, or [] if not found., Any (+28 more)
...
Cohesion: 0.07
Nodes (44): get_supported_variants(), is_variant_available_at(), Return variants supported by a specific DNA + formatter combination.      Applie, Check if a variant is available at a specific grade/quarter per curriculum., Any, Registry to map visual types to their Pydantic schemas., VisualSchemaRegistry, _parse_contract_section_refs() (+36 more)
...
Cohesion: 0.06
Nodes (49): Generate a single practice problem and return it as a dict.      This is the sin, run(), build(), main(), Any, Attester packets — the blind evidence a CAPABILITY_PROVIDERS entry has to surviv, Emit the Attester-facing text VERBATIM from a packet.      Blindness is a prompt, Returns (packets, key).      `packets` is the blind half: an opaque item id, the (+41 more)
...
Cohesion: 0.09
Nodes (19): Mutation tests for the capability contract (§6A/§6B/§6C).  These exist because g, The discrimination must key on *what survives removing the family*, never on how, The regression §6F exists to stop: someone re-registers what a blind Attester, §6F must clear once a verdict is on file, or it is a counter rather than a check, There is no 'partly provided'. A malformed record is loud, never skipped., A requirement the curriculum never states cannot be declared., Planted mutation: an entry whose ONLY possible carrier is the shared catch-all., At the cap it passes; one over, it fails. The threshold is a visible number. (+11 more)
...
Cohesion: 0.10
Nodes (15): get_dnas_for_formatter(), get_variant_curriculum_gate(), is_compatible(), Any, Practice Generation — DNA-Formatter Compatibility Table ========================, Validate a lab UI selection and return compatibility info.      Args:         dn, Return the curriculum introduction point for a variant, or None if no gate., Return True if the formatter is compatible with the given DNA concept.      Args (+7 more)
...
Cohesion: 0.12
Nodes (22): SessionLocal(), Background task to pre-generate questions into the cache.     Uses parallel exec, replenish_question_cache(), CompetencyConfiguration, Background task to pre-generate questions into the cache.     Uses parallel exec, replenish_question_cache(), Background task to pre-generate questions into the cache.     Uses parallel exec, replenish_question_cache() (+14 more)
...
Cohesion: 0.11
Nodes (26): _node_file(), Path, Practice Generation — Judgment Review Validator (hard gate)  The judgment items, Return a list of schema/quality errors for one node's review file., Collapse a rationale to the structure that survives per-node substitution., One reviewer identity may not stamp the whole tree.      Blind review is dispatc, Fail any normalized rationale skeleton shared by more than _MAX_SKELETON_CLUSTER, Validate every registered node's judgment review. Returns a flat list of     err (+18 more)

### Community 103 - "Community 103"
Cohesion: 0.06
Nodes (31): 1. Fractions answer-key false positive — `mat_g2_na_q4_0`/`_1`/`_2`, 2026-08-13 — length_measurement: the largest FAIL cluster in the tree, 2026-08-13 — The G1 measure items: a visual for the task, and a size model for the units, 2026-08-19 — Unit 3 completion + full `run_all` verification, 2026-08-20 (tick 8) — A dispatch-only tick: coverage 52 → 127, 2026-08-21 (tick 12) — An array cannot depict a zero factor, 2026-08-23 — §6G: an attestation must show its work, 2. `§1A-reach` false failure — `mat_g3_na_q2_0` (money_peso) (+23 more)
...
Cohesion: 0.15
Nodes (14): get_node_dnas(), Return the list of DNA concept names for a node.      Args:         node_id: MAT, PracticeOrchestrator, Any, Bridge Layer separating the generation logic from the FastAPI routing/server lay, compute(), main(), Returns (exclusions, broken) -- broken = pairs that raise a NON-eligibility erro (+6 more)
...
Cohesion: 0.09
Nodes (35): DNA, Specification of a mathematical concept for practice generation.      dna_type:, Return param_bounds for grade, falling back to nearest defined grade., Return the index of a level within an axis (0 = easiest)., Return a 0.0–1.0 scalar for a given axis level., enumerate_profiles(), extract_numerical_limits(), get_dimension_ranges() (+27 more)

### Community 183 - "1. Local Prototyping: Data Seeding"
Cohesion: 0.15
Nodes (20): _answer_leaks_into_stem(), _normalize_stem(), _numeric_payload_values(), _option_in_stem(), Any, Every plain number a generated problem exposes to the student, labelled for, Lowercase the stem and render LaTeX fractions as 'a/b' so an option like     '1/, Whether an option value appears in the stem as a *whole* token.      Substring m (+12 more)
...
Cohesion: 0.14
Nodes (20): _normalize(), _option_values(), _provenance_corpus(), Any, Collapse whitespace so re-rendered text compares on content, not layout., Re-render every seed the review cites and assert the review is still about     t, The offered option values as a sorted list of strings, or None when the sample, Everything a rationale for this node is entitled to quote: the samples the     r (+12 more)
...
Cohesion: 0.33
Nodes (10): _attestation_records(), Every attestation batch on disk, whole, for the freshness pass., _blindness(), build_records(), check_skeletons(), _load(), main(), Any (+2 more)
...
Cohesion: 0.14
Nodes (25): get_pipeline_status(), Return a health-check dict for the pipeline.      Checks:       - Whether each o, get_all_node_ids(), Return all node IDs from NODE_TO_DNA, optionally filtered.      Nodes are return, _bound_allows(), build_group(), build_packet(), _main() (+17 more)
...
Cohesion: 0.07
Nodes (42): _build_symbolic_question(), _detect_axes_served(), _eval_error_formula(), generate_context(), get_node(), Any, Practice Generation — Context Generator ========================================, Return the raw knowledge-graph node dict for node_id, or None.      Args: (+34 more)
...
## Knowledge Gaps
- **1205 isolated node(s):** `graphify`, `PackageDescription`, `Foundation`, `PathKit`, `graphify-mcp` (+1200 more)
...
- **Why does `FormattedProblem` connect `models.py` to `generate_params`, `_get_available_formats`, `.axis_level_index`, `Community 8`, `fmt_array_grid.py`, `Community 138`, `Community 13`, `Community 14`, `Community 18`, `fmt_fill_in_table.py`, `Community 41`, `Community 50`, `Community 51`, `Spine`, `Community 59`, `Community 60`, `get_gemini_models`, `Community 70`, `Community 77`, `Community 79`, `Community 92`, `Community 93`, `Community 95`, `get_all_node_ids`, `Community 99`, `Community 100`, `TestLogScaleDeclarations`, `Community 117`, `2026-08-13 — perimeter: an impossible triangle, and two shapes that never appeared`?**
  _High betweenness centrality (0.029) - this node is a cross-community bridge._
- **Why does `QuestionContext` connect `Community 50` to `generate_params`, `_get_available_formats`, `.axis_level_index`, `Community 138`, `Community 13`, `Community 18`, `_combined_interests`, `fmt_fill_in_table.py`, `Community 41`, `Community 51`, `Spine`, `Community 59`, `Community 60`, `get_gemini_models`, `Community 70`, `Community 77`, `Community 79`, `Community 92`, `Community 93`, `models.py`, `Community 95`, `get_all_node_ids`, `Community 99`, `Community 100`, `TestLogScaleDeclarations`, `2026-08-13 — perimeter: an impossible triangle, and two shapes that never appeared`?**
  _High betweenness centrality (0.028) - this node is a cross-community bridge._
- **Why does `get_node_info()` connect `_combined_interests` to `Community 4`, `Community 102`, `Community 104`, `Community 8`, `Community 9`, `Community 75`, `Community 11`, `2026-08-13 — length_measurement: the largest FAIL cluster in the tree`, `Community 14`, `Community 16`, `hardening_supervisor.py`, `Native SQL Examples`, `models.py`, `2026-08-13 — Content no competency asks for, and a metre wearing centimetres`?**
