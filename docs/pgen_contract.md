...
| Every assertion the harness can emit is either proven by a mutation naming it in `Mutation.asserts`, or listed in `UNPROVEN_ASSERTIONS` with a reason and a date; the allowlist may only SHRINK. The inventory is the WHOLE harness: `validate_matrix`'s labels are read from its AST (f-string families included), every other module declares `ASSERTIONS`, and four further directions pin those declarations — a mutation may not assert an undeclared label, the allowlist may not name one, an allowlisted label a mutation now proves must come off, and every `  FAIL <label>` a validator prints must be inventoried. 2026-09-08: 46/96 proven, 50 knowingly unproven (was 16/26 while 27 proven assertions sat outside the inventory entirely). BLIND SPOT: a check that neither declares `ASSERTIONS` nor prints a `  FAIL` line, and a second mutation on an already-proven label (§7's `mutations` floor guards that) | `validate_coverage` §8 | local `run_all`; not in CI |
