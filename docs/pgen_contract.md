...
| A discrete variant a node DECLARES must be one it can actually produce. **Floor 12 since 2026-09-08** (was 65). The 53 cleared were all declarations offering values the node's own competency or grade excludes — regrouping deeper than the digit ceiling, regrouping on estimation competencies, multiplication `tables` on addition nodes, variants a multi-DNA node's effective bounds forbid, and standard units on G1 "non-standard units" nodes. No generator changed; §7's `variant_candidates` floor guards against clearing these by over-filtering | `validate_compat` §2I | local `run_all`; not in CI |
