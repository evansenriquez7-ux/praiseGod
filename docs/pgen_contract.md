...
| A discrete variant a node DECLARES must be one it can actually produce. Baseline 2026-09-04: **65 unproducible across 33 nodes** (regrouping 38, tables 8, task_type 6, unit_type 6, number_type 4, strategy 3) — a FLOOR that may only shrink (Mandate #5). Two causes behind one number: a value impossible at the node's ceiling (narrow the declaration) vs a variant the DNA never implements for this shape (a §6 capability gap). Was invisible because `judgment_packets._try_render` swallowed every failure with `except Exception: return None`; failures are now recorded and surfaced | `validate_compat` §2I | local `run_all`; not in CI |
